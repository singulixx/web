...
export default function Page() {
  ...
  const statusLabel = (s: Ball["status"]) =>
    s === "UNOPENED" ? "Belum dibuka" : s === "OPENED" ? "Dibuka" : "Selesai sortir";
  ...
  const fetchBalls = async () => {
    ...
    try {
      ...
      // Unwrap: array langsung, atau envelope { data/items, total/meta.total }
  ...
  }, [token, page]); // penting: dengarkan perubahan page
  ...
  const handleSubmit = async (payload: BallForm & { nota?: File[] | null }) => {
    ...
    if (payload.beratKg !== undefined) fd.append("beratKg", String(payload.beratKg));
    if (payload.hargaBeli !== undefined) fd.append("hargaBeli", String(payload.hargaBeli));
    ...
    setSaving(true);
    try {
      const res = await fetch(payload.id ? `${BALLS_URL}/${payload.id}` : BALLS_URL, {
  ...
  return (
    <div className="space-y-4">
      {!token && <Alert message="Anda belum login. Silakan login untuk mengelola data Ball." />}
      {error && <Alert message={error} />}

      {!showBallForm && !editingBall && (
        <button className="btn btn-primary mb-4" onClick={() => setShowBallForm(true)}>
          + Tambah Ball
        </button>
      )}
      ...
      {(showBallForm || !!editingBall) && (
        <div className="card bg-base-100 shadow">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-sm text-slate-400">
                {editingBall ? "Perbarui data Ball yang dipilih" : "Input data Ball baru"}
              </p>
            </div>
            <button
          ...
          />
          {saving && <p className="text-sm text-slate-500 mt-2">Menyimpan...</p>}
        </div>
      )}
