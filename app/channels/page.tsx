...
type StatusMap = Record<number, { connected?: boolean; message?: string } | undefined>;
...
export default function ChannelPage() {
  ...
  // --- pagination state (sederhana) ---
  const [page, setPage] = useState(1);
  const pageSize = 9; // tampilkan 9 kartu per halaman (3 kolom x 3 baris)
  const total = stores.length;

  // filter per channel
  const shopeeStoresAll = useMemo(
  ...
  // terapkan pagination untuk daftar shopee (sebagai contoh; sesuaikan kalau mau untuk tiktok/offline juga)
  const shopeeStores = useMemo(() => {
    const start = (page - 1) * pageSize;
    return shopeeStoresAll.slice(start, start + pageSize);
  }, [shopeeStoresAll, page, pageSize]);
  ...
  // NB: kalau benar2 hanya owner yang boleh manage, gunakan === "OWNER".
  // Sekarang ini OWNER atau STAFF bisa manage.
        ...
        <div className="card bg-base-100 shadow p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Link href="/settings/stores" className="btn btn-ghost">
                Kelola Stores
              </Link>
          ...
          <section className="mt-6">
            ...
            {!loading && shopeeStoresAll.length === 0 && (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`badge ${connected ? "badge-success" : "badge-amber"}`}
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {tiktokStores.map((s) => (
                ...
                >
                  <div>
                    <div className="font-medium">{s.name}</div>
                    <div className="text-xs text-muted-foreground">TikTok (Manual)</div>
                  </div>
                  <Link href="/sales" className="btn btn-sm btn-ghost">
                    Tambah Transaksi
                  </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {offlineStores.map((s) => (
                ...
                >
                  <div>
                    <div className="font-medium">{s.name}</div>
                    <div className="text-xs text-muted-foreground">Offline (Manual)</div>
                  </div>
                  <Link href="/sales" className="btn btn-sm btn-ghost">
                    Tambah Transaksi
                  </Link>
