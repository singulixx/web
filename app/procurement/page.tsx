...
export default function Page() {
  ...
  // URL list bergantung pada page/pageSize => pakai useMemo
  const LIST_URL = useMemo(
    () =>
      `${API_BASE}/api/procurements?limit=${pageSize}&skip=${
        (page - 1) * pageSize
      }`,
  ...
  const normalizeList = (raw: any): { items: ProcurementRow[]; total: number } => {
    if (Array.isArray(raw)) return { items: raw as ProcurementRow[], total: raw.length };
    if (raw?.items && Array.isArray(raw.items))
      return { items: raw.items as ProcurementRow[], total: Number(raw.total ?? raw.items.length) };
    if (raw?.data && Array.isArray(raw.data))
      return { items: raw.data as ProcurementRow[], total: Number(raw.total ?? raw.data.length) };
  ...
  async function load() {
    ...
    try {
      ...
      const { items, total } = normalizeList(json);
      setRows(items);
      setTotal(total);
    } catch (e: any) {
      setRows([]);
  ...
  const confirmDelete = async () => {
    ...
    try {
      // Hapus pakai endpoint detail (bukan LIST_URL)
      const r = await fetch(`${API_BASE}/api/procurements/${confirm.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!r.ok) {
