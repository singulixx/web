  ...
  const connectShopee = async (id: number) => {
    try {
      const headers: HeadersInit | undefined = token
        ? { Authorization: `Bearer ${token}` }
        : undefined;
      // FIX endpoint: jangan tempel parameter limit/skip di tengah path
                ...
                <div key={s.id} className="card bg-base-100 shadow">
                  <div className="card-body gap-3 sm:flex sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="font-semibold truncate">Shopee: {s.name}</div>
                      <div className="text-sm opacity-70">
                        ...
                        {st.expiresAt && (
                          <span className="ml-2">
                            • Exp: {new Date(st.expiresAt).toLocaleString("id-ID")}
                          </span>
                        )}
                      </div>
                    </div>
        ...
        <div className="card bg-base-100 shadow h-fit">
          <div className="card-body">
            <div className="flex items-center justify-between mb-2">
              <h2 className="card-title">{selected ? "Edit Store" : "Tambah Store"}</h2>
              <button
                onClick={() => {
                  setShowForm(false);
                  setSelected(null);
            ...
            <FormStore
              ...
              onSubmit={(data) => (selected ? handleUpdate(data) : handleCreate(data))}
            />
          </div>
        </div>
      )}
