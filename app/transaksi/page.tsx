...
// Respons bisa berupa array langsung atau envelope { items, total }
type TxListResponse = Tx[] | { items: Tx[]; total?: number };
...
export default function Page() {
  ...
  const fetchTransactions = async () => {
    ...
    setLoading(true);
    try {
      // Disarankan: signature objek
      const res: TxListResponse = await api.Transactions.list({
        authToken: token,
        limit: pageSize,
        offset: (page - 1) * pageSize, // ganti ke 'skip' jika backend pakai ?skip=
        query: params,
      });

      // --- Alternatif (jika lib kamu masih token, params):
      // const res: TxListResponse = await api.Transactions.list(token, {
      //   ...params,
      //   limit: pageSize,
      //   skip: (page - 1) * pageSize, // atau offset
      // });

      const list = Array.isArray(res) ? res : (res.items ?? []);
      const totalCount = Array.isArray(res) ? list.length : (res.total ?? list.length);
