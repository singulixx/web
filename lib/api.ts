...
export const Account = {
  forgotStart(body: { username: string }) {
    return jsonFetch<{ ok: true; token?: string; expiresAt?: string }>(
      withBase(`/api/account/forgot/start`),
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }
    );
  },
  forgotComplete(body: { username: string; token: string; newPassword: string }) {
    return jsonFetch<{ ok: true }>(
      withBase(`/api/account/forgot/complete`),
      { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }
    );
  },
...
const Transactions = {
  list(opts?: {
    ...
    return jsonFetch<{ items: any[]; total: number }>(withBase(`/api/transactions${q}`), {
      authToken: opts?.authToken,
    });
  },
